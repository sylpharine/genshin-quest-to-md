...
## 功能
- 输出章节标题
- 输出 `storyList[].info.title` 与 `storyList[].info.description`
- 输出任务标题与描述
- 输出对话（角色：文本）
- 保留分支对话并在后续合并重复剧情
- 可替换旅行者名字（默认：旅行者）
...
模板可用变量示例：`{chapter_num}`, `{chapter_title}`, `{story_title}`, `{task_title}`, `{role}`, `{text}`, `{index}`。
...
- `tasks`: `[{story_id, story_title, story_desc, task_id, title, desc, nodes}]`
